...
module.exports =
class AtomMacaulay2View extends View
  animating: false
  id: ''
  maximized: false
  opened: false
  ...
  @content: ->
    @div class: 'atom-macaulay2 terminal-view', outlet: 'atomMacaulay2View', =>
  ...
  initialize: (@id, @pwd, @statusIcon, @statusBar, @shell, @args=[], @env={}, @autoRun=[]) ->
    ...
    @prevHeight = atom.config.get('atom-macaulay2.style.defaultPanelHeight')
    ...
    @setAnimationSpeed()
    @subscriptions.add atom.config.onDidChange 'atom-macaulay2.style.animationSpeed', @setAnimationSpeed

    @updateToolbarVisibility()
    @subscriptions.add atom.config.onDidChange 'atom-macaulay2.toggles.showToolbar', @updateToolbarVisibility

    override = (event) ->
      return if event.originalEvent.dataTransfer.getData('atom-macaulay2') is 'true'
    ...
    @xterm.on 'mouseup', (event) =>
      if event.which != 3
        ...
        if atom.config.get('atom-macaulay2.toggles.selectToCopy') and text
    ...
    if /zsh|bash/.test(@shell) and @args.indexOf('--login') == -1 and Pty.platform isnt 'win32' and atom.config.get('atom-macaulay2.toggles.loginShell')
      @args.unshift '--login'
  ...
  setAnimationSpeed: =>
    @animationSpeed = atom.config.get('atom-macaulay2.style.animationSpeed')
    @animationSpeed = 100 if @animationSpeed is 0

    @xterm.css 'transition', "height #{0.25 / @animationSpeed}s linear"

  updateToolbarVisibility: =>
    @showToolbar = atom.config.get('atom-macaulay2.toggles.showToolbar')
    if @showToolbar
  ...
  displayTerminal: ->
    ...
    @terminal = new Terminal {
      cursorBlink     : false
      scrollback      : atom.config.get 'atom-macaulay2.core.scrollback'
      cols, rows
    }
  ...
  attachListeners: ->
    @ptyProcess.on "atom-macaulay2:data", (data) =>
      @terminal.write data

    @ptyProcess.on "atom-macaulay2:exit", =>
      @destroy() if atom.config.get('atom-macaulay2.toggles.autoClose')
    ...
    @ptyProcess.on "atom-macaulay2:title", (title) =>
      @process = title
    ...
    @terminal.once "open", =>
      ...
      autoRunCommand = atom.config.get('atom-macaulay2.core.autoRunCommand')
  ...
  open: =>
    ...
    @onTransitionEnd =>
      if not @opened
        ...
        @emit "atom-macaulay2:terminal-open"
      else
        @focus()
  ...
  pty: () ->
    if not @opened
      wait = new Promise (resolve, reject) =>
        @emitter.on "atom-macaulay2:terminal-open", () =>
          resolve()
  ...
  ptyPromise: () ->
    new Promise (resolve, reject) =>
      if @ptyProcess?
        @ptyProcess.on "atom-macaulay2:pty", (pty) =>
          resolve(pty)
  ...
  applyStyle: ->
    config = atom.config.get 'atom-macaulay2'

    @xterm.addClass config.style.theme
    
    @subscriptions.add atom.config.onDidChange 'atom-macaulay2.style.theme', (event) =>
    ...
    @subscriptions.add atom.config.onDidChange 'editor.fontFamily', (event) =>
      ...
      @terminal.element.style.fontFamily = overrideFont or editorFont or defaultFont
    @subscriptions.add atom.config.onDidChange 'atom-macaulay2.style.fontFamily', (event) =>
    ...
    @subscriptions.add atom.config.onDidChange 'editor.fontSize', (event) =>
      ...
      @terminal.element.style.fontSize = "#{overrideFontSize or editorFontSize}px"
      @resizeTerminalToView()
    @subscriptions.add atom.config.onDidChange 'atom-macaulay2.style.fontSize', (event) =>
  ...
  insertSelection: (customText) ->
    ...
    runCommand = atom.config.get('atom-macaulay2.toggles.runInsertedText')
    selectionText = ''
  ...
  getTitle: ->
    @statusIcon.getName() or "atom-macaulay2"
